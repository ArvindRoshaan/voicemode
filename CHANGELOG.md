...
- **Opt-in manual stop for `converse` (end your turn on your terms)** — Two new intent-named options let you control when a recording ends instead of relying on silence detection: **`manual_stop`** (recording ends only when you press a key/ESC or hit `listen_duration_max` — VAD silence auto-stop is off, so pauses never cut you off) and **`manual_stop_with_silence_detection`** (a key/ESC **or** a silence pause ends it, whichever comes first). Both are available on the `converse` tool and the CLI (`--manual-stop` / `--manual-stop-with-silence-detection`), and both default off. **Strictly opt-in — default behavior is unchanged** (classic VAD; ESC = plain cancel, "Cancelled by user."), preserving the VM-1026/#337 fix that keeps the MCP connection alive on cancel. The stop key is surface-appropriate: in a **real terminal** (the `voicemode converse` CLI) you press **Enter/Space** (read from `/dev/tty`); in **Claude Code** you press **ESC** (the only key the harness delivers to a running tool). If no controlling terminal is available, it falls back to VAD/max-duration. In Claude Code, when you press **ESC** to end a manual-stop turn, the audio captured so far is transcribed (it survives the cancellation via a shared in-memory buffer, since the recording thread isn't directly cancellable). Because the Claude Code harness discards a cancelled tool's return value, the transcript is written to `~/.voicemode/last_esc_transcript.txt` and a companion **`/get-transcript`** skill brings your spoken words into the conversation (the harness offers no hook to do this automatically on ESC). A **`/converse-manual-stop`** skill is also included as a convenience launcher that starts a conversation with `manual_stop` already enabled and documents the ESC → `/get-transcript` flow. (Skills ship with the Claude Code **plugin**, not the PyPI wheel.) **Transport note:** the ESC recovery runs briefly in the background after the tool is cancelled; over **stdio**, Claude Code does not auto-reconnect an MCP server after a cancellation, so the *next* converse call may fail with "Connection closed" until you reconnect (`/mcp` → Reconnect). Running VoiceMode over **HTTP** (`voicemode serve` + `claude mcp add --transport http`) avoids this — Claude Code auto-reconnects HTTP/SSE servers — so HTTP transport is recommended for the smoothest in-chat manual-stop experience. (The CLI keypress path is unaffected and reliable on its own.)
